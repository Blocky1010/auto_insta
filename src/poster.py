...
def post_reel(api, reel):
    # Extraire la première ligne de la description
    first_line_of_caption = reel.caption.split('\n')[0] if '\n' in reel.caption else reel.caption

    # Construire la nouvelle légende
    new_caption = f"Video by: @{reel.account}\n{first_line_of_caption}\n\n{config.HASHTAGS}"

    # Publier le Reel avec la légende modifiée
    api.clip_upload(reel.file_path, new_caption)

    # Marquer le Reel comme posté
    reel.is_posted = True
